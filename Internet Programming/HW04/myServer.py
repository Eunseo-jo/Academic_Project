...
class HTTPServer:
    ...
    def get_request(self, requested_file, data):
        ...
        if self.head_request == NOT_FOUND: return self.resource_not_found()
        elif self.head_request == FORBIDDEN: return self.resource_forbidden()
        else:
            ext = os.path.splitext(requested_file)[1]
            if should_return_binary(ext) == True: 
                mime = get_file_mime_type(ext) 
                return get_file_binary_contents(requested_file)
            else: return get_file_contents(requested_file)
    ...
    def post_request(self, requested_file, data):
        ...
        """
        soup = BeautifulSoup(requested_file)
    ...
    def resource_not_found(self):
        ...
        mime_types = get_file_mime_type("html")
        builder.add_header("Content-Type", mime_types)
        builder.add_header("Connection", "close")
...
class ResponseBuilder:
    ...
    def build(self):
        ...
        Uses the `self.status` 200 , `self.headers` and `self.content` to form
